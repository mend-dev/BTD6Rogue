...
public class MonkeyAceArtifact : RogueArtifact {
    public override string DisplayName => "Dart Monkey Training";
    public override string ArtifactSprite => VanillaSprites.MonkeyAce000;
    ...
    public override void OnChooseArtifact(InGame game) {
        foreach (Tower tower in game.GetTowers()) {
            if (tower.GetMutator("MonkeyAceArtifact") != null) { tower.RemoveMutatorsById("MonkeyAceArtifact"); }
            if (tower.towerModel.baseId == TowerType.MonkeyAce) {
                BehaviorMutator bm = new DamageSupport.MutatorTower(1f, false, "MonkeyAceArtifact", new BuffIndicatorModel("", "", ""));
                tower.AddMutatorIncludeSubTowers(bm, 9999999);
                BehaviorMutator bm2 = new RateSupportModel.RateSupportMutator(false, "MonkeyAceArtifact", 0.75f, 0, new BuffIndicatorModel("", "", ""));
    ...
    public override bool CanGetArtifact(InGame game) {
        foreach (Tower tower in game.GetTowers()) {
            if (tower.GetMutator("MonkeyAceArtifact") != null) { return false; }
            if (tower.towerModel.baseId == TowerType.MonkeyAce) { return true; }
        }
        return false;
    }
}