...
public class IceMonkeyArtifact : RogueArtifact {
    public override string DisplayName => "Dart Monkey Training";
    public override string ArtifactSprite => VanillaSprites.IceMonkey000;
    ...
    public override void OnChooseArtifact(InGame game) {
        foreach (Tower tower in game.GetTowers()) {
            if (tower.GetMutator("IceMonkeyArtifact") != null) { tower.RemoveMutatorsById("IceMonkeyArtifact"); }
            if (tower.towerModel.baseId == TowerType.IceMonkey) {
                BehaviorMutator bm = new DamageSupport.MutatorTower(1f, false, "IceMonkeyArtifact", new BuffIndicatorModel("", "", ""));
                tower.AddMutatorIncludeSubTowers(bm, 9999999);
                BehaviorMutator bm2 = new RateSupportModel.RateSupportMutator(false, "IceMonkeyArtifact", 0.75f, 0, new BuffIndicatorModel("", "", ""));
    ...
    public override bool CanGetArtifact(InGame game) {
        foreach (Tower tower in game.GetTowers()) {
            if (tower.GetMutator("IceMonkeyArtifact") != null) { return false; }
            if (tower.towerModel.baseId == TowerType.IceMonkey) { return true; }
        }
        return false;
    }
}