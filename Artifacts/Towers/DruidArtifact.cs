...
public class DruidArtifact : RogueArtifact {
    public override string DisplayName => "Dart Monkey Training";
    public override string ArtifactSprite => VanillaSprites.Druid000;
    ...
    public override void OnChooseArtifact(InGame game) {
        foreach (Tower tower in game.GetTowers()) {
            if (tower.GetMutator("DruidArtifact") != null) { tower.RemoveMutatorsById("DruidArtifact"); }
            if (tower.towerModel.baseId == TowerType.Druid) {
                BehaviorMutator bm = new DamageSupport.MutatorTower(1f, false, "DruidArtifact", new BuffIndicatorModel("", "", ""));
                tower.AddMutatorIncludeSubTowers(bm, 9999999);
                BehaviorMutator bm2 = new RateSupportModel.RateSupportMutator(false, "DruidArtifact", 0.75f, 0, new BuffIndicatorModel("", "", ""));
    ...
    public override bool CanGetArtifact(InGame game) {
        foreach (Tower tower in game.GetTowers()) {
            if (tower.GetMutator("DruidArtifact") != null) { return false; }
            if (tower.towerModel.baseId == TowerType.Druid) { return true; }
        }
        return false;
    }
}