...
public class MortarMonkeyArtifact : RogueArtifact {
    public override string DisplayName => "Dart Monkey Training";
    public override string ArtifactSprite => VanillaSprites.MortarMonkey000;
    ...
    public override void OnChooseArtifact(InGame game) {
        foreach (Tower tower in game.GetTowers()) {
            if (tower.GetMutator("MortarMonkeyArtifact") != null) { tower.RemoveMutatorsById("MortarMonkeyArtifact"); }
            if (tower.towerModel.baseId == TowerType.MortarMonkey) {
                BehaviorMutator bm = new DamageSupport.MutatorTower(1f, false, "MortarMonkeyArtifact", new BuffIndicatorModel("", "", ""));
                tower.AddMutatorIncludeSubTowers(bm, 9999999);
                BehaviorMutator bm2 = new RateSupportModel.RateSupportMutator(false, "MortarMonkeyArtifact", 0.75f, 0, new BuffIndicatorModel("", "", ""));
    ...
    public override bool CanGetArtifact(InGame game) {
        foreach (Tower tower in game.GetTowers()) {
            if (tower.GetMutator("MortarMonkeyArtifact") != null) { return false; }
            if (tower.towerModel.baseId == TowerType.MortarMonkey) { return true; }
        }
        return false;
    }
}