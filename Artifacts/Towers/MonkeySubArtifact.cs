...
public class MonkeySubArtifact : RogueArtifact {
    public override string DisplayName => "Dart Monkey Training";
    public override string ArtifactSprite => VanillaSprites.MonkeySub000;
    ...
    public override void OnChooseArtifact(InGame game) {
        foreach (Tower tower in game.GetTowers()) {
            if (tower.GetMutator("MonkeySubArtifact") != null) { tower.RemoveMutatorsById("MonkeySubArtifact"); }
            if (tower.towerModel.baseId == TowerType.MonkeySub) {
                BehaviorMutator bm = new DamageSupport.MutatorTower(1f, false, "MonkeySubArtifact", new BuffIndicatorModel("", "", ""));
                tower.AddMutatorIncludeSubTowers(bm, 9999999);
                BehaviorMutator bm2 = new RateSupportModel.RateSupportMutator(false, "MonkeySubArtifact", 0.75f, 0, new BuffIndicatorModel("", "", ""));
    ...
    public override bool CanGetArtifact(InGame game) {
        foreach (Tower tower in game.GetTowers()) {
            if (tower.GetMutator("MonkeySubArtifact") != null) { return false; }
            if (tower.towerModel.baseId == TowerType.MonkeySub) { return true; }
        }
        return false;
    }
}